...
export const BentoGridItem = ({
  ...
  const rightLists = ['TailwindCSS', 'Firebase', 'MongoDB'];

  const handleCopy = () => {
    const email = 'shannu2409@gmail.com';
      ...
      <div className={`${id === 6 && "flex justify-center"} h-full`}>
        <div className="w-full h-full absolute">
          {img && (
            <img
              src={img}
              alt={typeof img === 'string' ? img : 'bento grid item image'}
        ...
        <div className={`absolute right-0 -bottom-5 ${id === 5 && "w-full opacity-80"}`}>
          {spareImg && (
            <img
              src={spareImg}
              alt={typeof spareImg === 'string' ? spareImg : 'bento grid spare image'}
        ...
        >
          {id === 1 ? (
            <div className="w-full mt-auto flex flex-col justify-end h-full">
              <p className="lg:text-sm text-xs text-[#C1C2D3] font-sans mb-2 w-full">
                {title}
              </p>
            </div>
          ) : (
            <>
              ...
              {id === 3 && (
                <div className="flex gap-1 lg:gap-5 w-fit absolute -right-4 lg:-right-1">
